...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('ahmad.saad.khn@gmail.com')
    setCopied(true);
  }
            ...
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-1">
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-3">
                {['React.js', 'Next.js', 'TypeScript'].map((item) => (
                  <span
                    key={item}
                    className="py-2 lg:py-2 lg:px-2 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]"
                  >
                    {item}
                  </span>
                ))}
